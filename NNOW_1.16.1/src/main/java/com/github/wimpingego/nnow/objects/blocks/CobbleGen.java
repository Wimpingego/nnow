...
public class CobbleGen extends DirectionalBlock {
	...
	int ecost = ModConfigs.E_COST.get();
	int ereturn = ModConfigs.E_RETURN.get();
...
//PICKAXE				
		...
		if (itemstack.getItem() == Items.NETHERITE_PICKAXE) {
			itemstack.damageItem(ncost, player, e -> e.sendBreakAnimation(hand));
			ItemStack itemback;
...
//NNOW HAMMERS
			...
			if (itemstack.getItem() == ItemList.N_HAMMER.get()) {
				itemstack.damageItem(ncost, player, e -> e.sendBreakAnimation(hand));
				ItemStack itemback;
				itemback = new ItemStack(Items.COBBLESTONE, nreturn*2);
			...
			if (itemstack.getItem() == ItemList.E_HAMMER.get()) {
				itemstack.damageItem(ecost, player, e -> e.sendBreakAnimation(hand));
				ItemStack itemback;
				itemback = new ItemStack(Items.COBBLESTONE, ereturn);
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
	{
		...
		tooltip.add((new TranslationTextComponent("item.nnow.cobblegen.line1")));
		tooltip.add((new TranslationTextComponent("item.nnow.cobblegen.general.line1")));
	}
}
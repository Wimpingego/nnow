...
// STONE DUST FROM DIRT
	public ActionResultType onItemUse(ItemUseContext context) {		
		if (context.getWorld().getBlockState(context.getPos()).isIn(Tags.Blocks.DIRT)) {
			for (ItemStack stack : context.getPlayer().inventory.mainInventory) {
				if (stack.isEmpty()) {
					context.getPlayer()
							.addItemStackToInventory(new ItemStack(ItemList.STONE_DUST.get(), 0 + random.nextInt(3)));
...
// Sapling & Cactus DROPS
	...
	public boolean onBlockDestroyed(ItemStack stack, World world, BlockState state, BlockPos pos, LivingEntity entity) {
		...
		boolean sand = state.isIn(Tags.Blocks.SAND);

...
public class ModConfigs {
    ...
    private static class Bookshelf
    {
    	Bookshelf(ForgeConfigSpec.Builder builder)
		{
        	...
        	ENCHANT_POWER_STAIRS = builder.comment("Enchant power level modifier for the bookshelf stairs block.").defineInRange("Stair Enchant Level", 8, 0,15);
        	ENCHANT_POWER_SLABS = builder.comment("Enchant power level modifier for the bookshelf slabs block.").defineInRange("Slab Enchant Level", 1, 0,8);
        	builder.pop();
        }
    }
    ...
    private static class Poking_Stick
    {
    	Poking_Stick(ForgeConfigSpec.Builder builder)
    	{
			...
			DUST_DROP = builder.define("Dust drop amount modifier", 3);
